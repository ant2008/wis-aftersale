 ...
 */
@Component
public class PostPrtInterceptor implements HandlerInterceptor {

    private final String Jwt_Auth = "Authorization";

    private Logger LOG = LoggerFactory.getLogger(PostPrtInterceptor.class);
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        try {
            ...
            //根据方法名称判断不同的权限。
            //修改
            if (methodName.toLowerCase().trim().equals("exportexcel") ||
                    methodName.toLowerCase().trim().equals("ExportExcelAll".toLowerCase()) ||
                    methodName.toLowerCase().trim().equals("GetExportAllData".toLowerCase()) ||
                    methodName.toLowerCase().trim().equals("DownExcel".toLowerCase()) ||
                    methodName.toLowerCase().trim().equals("Prt".toLowerCase()) ||
                    methodName.toLowerCase().trim().equals("PrePrt".toLowerCase())) {
                if (vFuncRight.getPrtr() == null || vFuncRight.getPrtr() == 0) {
                    LOG.debug("用户ID{}没有{}的查询权限", userid, funcNo.toLowerCase());
                    throw new Exception("用户ID:" + userid + "没有[" + funcNo + "]的打印权限!");
