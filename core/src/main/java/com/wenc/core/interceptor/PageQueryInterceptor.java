 ...
 */
@Component
public class PageQueryInterceptor implements HandlerInterceptor {


    private final String Jwt_Auth = "Authorization";


    private Logger LOG = LoggerFactory.getLogger(PageQueryInterceptor.class);
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        try {
            ...
            //根据用户ID进行校验
            vFuncRight = userRightService.getFuncByUseridAndFuncno(userid, funcNo.trim());

            if (vFuncRight == null) {
                //LOG.debug("用户ID{}没有{}的权限",userid,funcNo.toLowerCase());
                throw new Exception("用户ID:" + userid + "没有[" + funcNo + "]的权限!");

            }

            if (vFuncRight.getQuyr() == null || vFuncRight.getQuyr() == 0) {
                LOG.debug("用户ID{}没有{}的查询权限", userid, funcNo.toLowerCase());
                throw new Exception("用户ID:" + userid + "没有[" + funcNo + "]的查询权限!");

            }

            //vFuncRight = userRightService.getFuncByUseridAndFuncno(userid,"addR")
